...
export function DashboardWideMapSection({ dateRange }: { dateRange: string }) {
  const lgUp = useLgUp();
  ...
  // Geocode & fly-to using Mapbox Geocoding API (no backend needed)
  ...
  return (
    <section className="flex min-h-[min(560px,calc(100vh-10rem))] flex-col overflow-hidden rounded-2xl border border-captionDark/20 bg-surface-light dark:border-captionDark-dark/20 dark:bg-surface-dark shadow-sm lg:min-h-[min(86dvh,56rem)] lg:flex-row">
      ...
      <div
        className={cn(
          ...
          "border-b lg:border-b-0 lg:border-r h-[min(62vh,460px)] w-full lg:h-full",
        )}
        style={leftStyle}
      >
        {/* Resize grip – desktop */}
      ...
      {/* ── MAP ───────────────────────────────────────────────── */}
      <div className="relative min-h-[min(64dvh,520px)] w-full min-w-0 flex-1 lg:min-h-0">

        {/* ── TOP OVERLAY ROW 1: Search bar ── */}
        ...
        {/* ── TOP OVERLAY ROW 2: Style + Filter chips ── */}
        ...
        {/* ── RIGHT SIDE: Floating detail card overlay ── */}
        {selectedRow && (
          <div
            className="pointer-events-auto absolute bottom-4 right-4 top-4 z-[510] flex w-[min(320px,calc(100%-2rem))] animate-inbox-card-enter flex-col overflow-hidden rounded-2xl border border-captionDark/20 bg-surface-light shadow-2xl dark:border-captionDark-dark/25 dark:bg-surface-dark"
          >
            <IncidentDetailCard
              row={selectedRow}
              embedded
