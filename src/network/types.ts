...
export interface MutationConfig<TVariables, TData> {
   ...
   */
  method?: "post" | "put" | "delete" | "patch" | "postForm" | "putForm" | "patchForm";
   ...
   */
  transformRequest?: (variables: TVariables) => Record<string, unknown> | FormData;
...
export interface QueryConfig<TParams, TData> {
  ...
  /**
   * Optional field to specify the name of the meta field in the API response.
   * This is useful for extracting the meta object from the response on the calls that we would make for traditional pagination
   */
  appendCompanyIdToRequest?: boolean;
}
...
/**
 * Default API response interface that represents the standard structure
 * returned by API endpoints across the application.
 *
 */
